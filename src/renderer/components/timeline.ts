...
import { TimelineComponent, TimelineEvent } from '../../types/viewspec.js';
...
function renderTimelineEvent(event: TimelineEvent): string {
  ...
  const statusBadge = event.status
    ? `<span class="timeline-event-status timeline-event-status-${event.status}">${event.status.replace('-', ' ')}</span>`
    : '';
